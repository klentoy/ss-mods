<?php
//Credit: iseulde, ivanrf https://github.com/iseulde/rename-wp-login - v2.5.5 - adjusted here by Rick Moore 22/2/19
	...
	class Rename_WP_Login {
		...
		private function new_login_slug() {
			if (
				...
				( $slug = 'backend' ) //*****CHANGE ME TO CHANGE THE DEFAULT LOGIN PAGE!
			) {
				return $slug;
			}
		}
		...
		public function wpmu_options() {
			echo (
				'<h3>' .
					_x( 'Rename wp-login.php', 'Text string for settings page', 'ss-mods' ) .
				'</h3>' .
				'<p>' .
					__( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'ss-mods' ) .
				'</p>' .
				'<table class="form-table">' .
					'<tr valign="top">' .
						'<th scope="row">' .
							__( 'Networkwide default', 'ss-mods' ) .
						'</th>' .
						'<td>' .
		...
		public function admin_init() {
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'ss-mods' ) . '</label>',
		...
		public function rwl_section_desc() {
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				echo (
					'<p>' .
						sprintf(
							__( 'To set a networkwide default, go to %s.', 'ss-mods' ),
							'<a href="' . esc_url( network_admin_url( 'settings.php#rwl-page-input' ) ) . '">' .
								__( 'Network Settings', 'ss-mods' ) .
							'</a>'
						) .
					'</p>'
				);
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="notice notice-success is-dismissible"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'ss-mods' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}
		...
		public function plugins_loaded() {
			global $pagenow;
			load_plugin_textdomain( 'ss-mods' );
			if (
				! is_multisite() && (
				...
				wp_die( __( 'This feature is not enabled.', 'ss-mods' ), '', array( 'response' => 403 ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'ss-mods' ), '', array( 'response' => 403 ) );
			}
			$request = parse_url( $_SERVER['REQUEST_URI'] );
			if (
				$pagenow === 'wp-login.php' &&
